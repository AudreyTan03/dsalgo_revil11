...
const ReviewScreen = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const response = await fetch(`https://revilcpe201-6018f6621dc2.herokuapp.com/api/reviews/?product=${id}`);
        if (!response.ok) {
  ...
  const submitReview = async () => {
    try {
      ...
      const response = await axios.post('https://revilcpe201-6018f6621dc2.herokuapp.com/api/post-reviews/', {
        product: parseInt(id),
        rating,
        comment,
